...
| **E10** External-review fix + same-class audit sweep → v0.1.3 (2026-08-25) | **PASS — reviewer's finding fixed; 2-round audit complete; every finding fixed; released and registry-verified.** | External X review found `env_use` consent bound to displayed argv, not target content. Fixed (3a1d920): every argument naming a readable file SHA-256 fingerprinted before the dialog (fingerprints shown in it) and re-hashed immediately pre-spawn; mismatch → new `SEP_TARGET_CHANGED`, nothing executes, nothing cached. Proven by tests incl. mutation-between-approval-and-spawn refusal and settle-then-reprompt. Same-class audit (2 time-boxed subagents, findings verified against source before fixing) then closed in 711e7ff: hook denylist bypasses (wrapper/path-prefix readers, mid-command `< file` redirects, glob suffixes, printf/declare-p/here-string var leaks — with a fail-open regression caught by a 31-payload probe of the built bundle, now pinned in `.commandcode/probe-audit-fixes.mjs`); detector suppression buckets hardened (digest charset+length floors so `sha256:` labels can't hide sk-keys, SRI-shape required after `integrity=`, data-URI MIME/base64 validation, path bucket needs real path structure, URL bucket refuses credential-shaped query params); consent surfaces unified on shared `core/display.ts` with bidi/unicode-separator escaping (twins byte-identical again), loopback links show destination host, TEST_MODE stub announces every answered key, no-TTY error no longer advertises the ASSUME_YES bypass. Gates: full monorepo suite green, build/typecheck/lint clean on tracked code, plugin contract 165/165, detector 44/44. Release run [32815056545](https://github.com/NDilanka/envseal/actions/runs/32815056545) green incl. publish; all 9 packages live at 0.1.3 with SLSA v1 provenance; clean-machine smoke on the published artifact: install (108 pkgs) → `envseal version 0.1.3` → shipped `core/dist/display.js` present → status shows key present with fingerprint → `env_use` injects and child output returns `«redacted:PROBE_KEY»`, `redactedCount: 1`. |
| **E11** Security hardening review → v0.1.4 (2026-08-25) | **PASS — released and registry-verified.** | Whole-repo review findings landed in [PR #1](https://github.com/NDilanka/envseal/pull/1) (`fa80cff`). Lockstep bump `0.1.3` → `0.1.4`. Release run [32848761172](https://github.com/NDilanka/envseal/actions/runs/32848761172) published all 9 `@envseal/*` packages; `@envseal/cli@0.1.4` is `latest` with SLSA v1 provenance. Residuals unchanged: Darwin keychain argv, hook fail-open default, MCP stdio host-trust. |
| **E12** Init host wiring (T10.1) → v0.1.5 (2026-08-25) | **PASS — implementation + docs; registry pending tag.** | `envseal init` always merges Layer 1 `AGENTS.md` and writes project-scoped MCP (`npx -y @envseal/mcp-server`, `npx.cmd` on Windows) for every matching project marker or `--host`. Does not crawl `$HOME`. Doctor fail-closes on unwired MCP / missing instructions (`agentWiring`). `--host` is a write override, not a fake protection tier. Lockstep bump `0.1.4` → `0.1.5`. Release run TBD until tag `v0.1.5` publishes. |
| **E13** Second external X review response — reads / injection-use / limited domains / auditing (`docs/verification/W9-comment-redteam.md`, 2026-08-25) | **PASS — every fixable finding closed with tests; boundaries documented; artifact probes green.** A second X reviewer asked: can the broker block reads; can it stop prompt injection using its own hand (echo an Authorization header); where are limited domains and auditing. Answers in code, 24 commits `9ec8c99..HEAD`: **Auditing** — `use`/`use_result` records around spawn (42ac954), wired through broker.use so the product path is covered (de4f9db); sha256-chained log + `envseal audit --verify` exit 7 with blame semantics and a tested-and-documented tail-truncation boundary (dec78a5); `declare`/`verify` events actually written for the first time — the extended zero-leak harness caught that they existed only as types (d188404). **Limited domains** — SEP/1.1 `policy.egress {warn\|allowlist}` manifest section with parse-time validation (76d80a3); anchored-wildcard enforcement refusing off-list and undeterminable hosts BEFORE any dialog (`SEP_EGRESS_DENIED`, de4f9db); network targets rendered in every consent dialog, doctor reports policy (517ab33). **Injection→use** — declared-secret variables denied in ANY argv position incl. `process.env.NAME`, closing the reviewer's literal curl-header example at hook level (f9b659f), layered under existing content-bound consent. **Reads** — hook closures: `-c`/eval recursion (9ec8c99), interpreters incl. paren-fragment rejoin (77348de), copiers/encoders/printers (35c0b4c), git object reads (4eb7bbc), fd/open-read-write redirects (fa70233), glob fuzz ≤2 edits + case folding off-Linux + cwd-sweep denial with rg recursive-by-default fix (b78e771), Windows shells + `/proc/*/environ` (7d41823), fail-mode loudness + opt-in `ENVSEAL_HOOK_FAIL_CLOSED` (5dca9dd), Grep/Glob routed through the matcher after the decide()-only version proved unreachable end-to-end (338ce47, 0e41864). **Gates:** full workspace build green; suites green across core 338/cli 120/plugin 264/mcp-server/sdk/protocol/registry/detector/prompters/http-server (zero ELIFECYCLE failures); lint clean on tracked code (remaining errors are untracked marketing scratch); artifact-level probes through the shipped bundle: `cat .env` → deny, `Glob **/.env*` → deny. Honest opens recorded in W9: MCP filesystem servers outside any matcher (advisory only), audit tail-truncation unfixable client-side, deliberate click-through exfiltration unchanged (residual §1). Landed via the hardening-wave merge; ships in the next version bump after v0.1.5. |
| **E14** Gauntlet-armed continuous hardening → v0.1.6 (2026-09-03) | **PASS — 3 features, 2 honesty fixes, release-gate parity fix; released and registry-verified.** | Engineering framework (poteto-mode/pstack) adopted; agent-validator ("gauntlet") armed as the pre-push gate (`pnpm gauntlet`, `.githooks/pre-push`, e1edb7d) with lint gate tightened to `--max-warnings 0` to match the release workflow (24e4be0). Shipped: **audit mirror** closes residual §10 tail-truncation with a chain-anchored out-of-band check under `~/.envseal/mirrors/` (ee369c2); **hook liveness heartbeat** — the PreToolUse hook records `.envseal/hook-heartbeat` (≤1/min, observational, fail-open preserved) and `doctor` reports when the hook last ran, closing §11's observability half (27a0203); **rotationDue** — the inert `rotation.maxAgeDays` knob now produces a real due date from the stored value's fingerprint age (re-stamped on change; `status` JSON + human overdue marker, `doctor` advisory list that never changes exit code), no policy/value/age stays honestly null (236b12a); **staging-temp self-ignore** — the §7 sibling fallback appends `..env.*.tmp` to the project .gitignore so a crash leftover is not stageable (29988c7); **Windows honesty fix** — `doctor` no longer reports `permissionsOk:false` on every Windows machine from Node's fake mode bits; now null-on-win32, pinned by a platform-aware test incl. explicit 0600/0644 modes (ff1c58a, 6c93eda). **Release:** two failed preflights were real gates working — a CI-only umask assumption in the new permissions test and the lint warning the old gauntlet tolerated; tag recovered both times pre-publish (version not burned). Final Release run [33676018907](https://github.com/NDilanka/envseal/actions/runs/33676018907) green; all 9 `@envseal/*` packages live at 0.1.6; `@envseal/cli@0.1.6` carries SLSA v1 `dist.attestations` provenance. Local clean-dir smoke: `npm view @envseal/<p> version` → 0.1.6 for all 9. |
