...
| **E11** Security hardening review → v0.1.4 (2026-08-25) | **IN PROGRESS — code merged; npm publish via `v0.1.4` tag.** | Whole-repo review findings landed in [PR #1](https://github.com/NDilanka/envseal/pull/1) (`fa80cff` on main). Patch release: nine `@envseal/*` packages `0.1.3` → `0.1.4`. Does not close Darwin keychain argv residual, hook fail-open default, or MCP stdio host-trust. |
