...
| T2 | Model shells out: `cat .env`, `echo $KEY`, `printenv`, `env`, `grep -r sk-` | Companion `PreToolUse` hook denies file reads matching `.env*`, `*.pem`, `*.key` patterns and denies commands matching env-dumping patterns (printenv, bare env, export -p, cat/head/tail/less on secret paths, echo $VAR, grep -r with secret pattern). Since the W9 round it also denies: interpreter code naming secret paths (`python3 -c "open('.env')"`), `sh -c`/`eval` payloads (recursed, depth-capped), git object reads of secret paths (`git show HEAD:.env`), copiers/encoders/printers (`cp`, `dd if=`, `base64`, `jq`, …), a DECLARED secret referenced in ANY argv position including `process.env.NAME` inside interpreter payloads, fd-number redirects, glob-fuzz names within edit distance 2, case-insensitive matches off Linux, recursive all-file sweeps, Windows shell wrappers, and `/proc/*/environ`. The hook is a **best-effort name-based denylist, not a sandbox**: it matches command text, fails open on its own internal errors, and cannot see non-Bash tools it is not wired to (native Grep/Glob, MCP filesystem servers — see W9 for the full bypass-class analysis). **Code:** `plugins/claude-code/hooks/pre-tool-use.ts` contains the deny rules. Denial messages are instructive, telling the model which tool to use instead. |
...
| T7 | `.env` gets committed to git | Before writing, the broker checks that `.env` is not git-tracked and that `.gitignore` covers it; if either check fails it refuses with `SEP_GITIGNORE_UNSAFE` and writes nothing. The refusal is unconditional — there is no offer-to-add-to-gitignore prompt and no override flag wired up today. **Code:** `packages/core/src/sinks/dotenv.ts` (`assertGitSafe`) performs the checks before any write. |
...
| T11 | Prompt injection in repo content drives `env_request` + exfiltration | The prompter popup always shows the project path, the model's verbatim `reason` string, and the target sink. `env_use` requires user confirmation for every command and displays the full command; commands with detected network egress add an explicit warning on top. Since 0.1.3, consent binds to **content**, not just displayed text: every argument naming a readable file is SHA-256 fingerprinted before the dialog is drawn (the dialog shows path + hash), and every fingerprint is re-verified against fresh disk content immediately before spawn — any mismatch refuses with `SEP_TARGET_CHANGED` and nothing executes, closing the window where injected repo content mutates a script after the user read the dialog. Residual risks remain: a user who reads the confirmation dialog and clicks through still defeats the control (inherent to any system where an agent can execute code); arguments that do not name readable files (PATH-resolved executables) stay name-approved; and the re-check narrows but cannot fully close the check-to-use race without handing an fd to the OS loader. **Code:** `packages/core/src/broker.ts` passes reason verbatim; `packages/core/src/exec.ts` detects egress (curl, wget, nc, ssh, HTTP tools), snapshots file fingerprints (`snapshotNamedFiles`) at approval and pre-spawn (`assertUnchanged`), and prompts; dialogs render the fingerprints (`useConfirmationBody` in both confirm twins). See residual risks. |
...
| T15 | Prompt injection drives a network command that echoes the secret (Authorization header to an attacker host) | Two independent layers. **Before the hook:** the tier-A PreToolUse hook denies any command referencing a DECLARED secret variable in any argument position (`curl -H "Authorization: Bearer $KEY" …`, `process.env.NAME` in interpreter payloads) — the value cannot reach the command line at all. **At the broker:** every `env_use` needs per-command consent; projects may set `policy.egress {mode: allowlist}` so any network target off the list refuses pre-dialog (`SEP_EGRESS_DENIED`), anchored wildcards, undeterminable hosts (bare IPs, encoded targets) refused by design; consent dialogs show the extracted network targets. A user who reads all of this and still approves sends the key — residual-risk §1, unchanged. **Code:** `plugins/claude-code/hooks/pre-tool-use.ts` (argv gate), `packages/core/src/egress.ts` + `exec.ts` (policy enforcement), SPEC/1.1 §Egress Policy. |
| T16 | Agent tampers with the audit log to hide what it did | Every audit record chains `sha256(previous raw line)` plus a sequence number; edits to surviving records break their successor's attestation and deletions surface as sequence gaps. `envseal audit --verify` reports the first break (exit 7). Blame semantics: content tampering blames the edited record (detected via k+1); deletion blames the missing position. Honest boundary: tail truncation is NOT detectable client-side — the chain proves survivors intact and ordered, not that history is complete, and an agent that can run shell can delete the file outright. This is tamper *evidence*, not immutability; an out-of-band sink is future work. **Code:** `packages/core/src/audit.ts` (`GENESIS_PREV`, `verifyAuditChain`), CLI `envseal audit`. See W9 GAP-AUDIT-3 for the tested truncation case. |
| T17 | Project policy forbids some hosts but the user cannot see what a command will touch | Consent dialogs render the command's extracted egress targets (`Network targets: api.openai.com`, or the undetermined-host form) whenever the command can reach the network, and `envseal doctor` prints the project's effective egress policy. Under allowlist mode every shown target has already passed the policy gate before the dialog exists. **Code:** onConfirm payload carries `egressHosts`; confirm surfaces in `@envseal/mcp-server` and `@envseal/sdk`; doctor in `@envseal/cli`. |
